document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!playerData || !playerData.name || !playerData.avatar) {
        console.error("Données du joueur invalides, redirection vers l'accueil.");
    ...
    // Ajout des événements pour la navigation et les modes de jeu
    ...
    // Déconnexion propre du socket avant de quitter la page
    window.addEventListener('beforeunload', () => {
        if (socket) {
            console.log("Déconnexion du socket avant de quitter la page.");
            socket.disconnect();
        }
    });
});